...
if ($LogAnalyticsWorkspaceId -and $LogAnalyticsPrimaryKey)
{
	...
	if ($HostpoolInfo -eq $null) {
		...
		$LogMessage = @{ hostpoolName_s = $HostpoolName; logmessage_s = "Hostpoolname '$HostpoolName' does not exist. Ensure that you have entered the correct values." }
		Add-LogEntry -LogMessageObj $LogMessage -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -logType "WVDTenantScale_CL" -TimeDifferenceInHours $TimeDifference
		exit
	}
	...
	if ($CurrentDateTime -ge $BeginPeakDateTime -and $CurrentDateTime -le $EndPeakDateTime) {
		UpdateLoadBalancerTypeInPeakandOffPeakwithBredthFirst -HostPoolName $HostpoolName -ResourceGroupName $ResourceGroupName -MaxSessionLimitValue $MaxSessionLimitValue -HostpoolLoadbalancerType $HostpoolLoadbalancerType -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -TimeDifference $TimeDifference
	}
	else {
		UpdateLoadBalancerTypeInPeakandOffPeakwithBredthFirst -HostPoolName $HostpoolName -ResourceGroupName $ResourceGroupName -MaxSessionLimitValue $MaxSessionLimitValue -HostpoolLoadbalancerType $HostpoolLoadbalancerType -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -TimeDifference $TimeDifference
	}
	...
	if ($CurrentDateTime -ge $BeginPeakDateTime -and $CurrentDateTime -le $EndPeakDateTime)
	{
		...
		foreach ($SessionHost in $ListOfSessionHosts) {
			$Name = $SessionHost.Name | Out-String
			$VMName = $Name.Split(".")[0].Split("/")[1]
			...
			Write-Output "Checking session host: $($SessionHost.Name | Out-String)  of sessions: $($SessionHost.Sessions) and status: $($SessionHost.Status)"
			$LogMessage = @{ hostpoolName_s = $HostpoolName; logmessage_s = "Checking session host: $($SessionHost.Name | Out-String)  of sessions:$($SessionHost.Sessions) and status:$($SessionHost.Status)" }
			Add-LogEntry -LogMessageObj $LogMessage -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -logType "WVDTenantScale_CL" -TimeDifferenceInHours $TimeDifference
			...
			foreach ($SessionHost in $AllSessionHosts.Name) {
				...
				if ($NumberOfRunningHost -lt $MinimumNumberOfRDSH) {
					...
					if ($SessionHost.ToLower().Contains($RoleInstance.Name.ToLower())) {
						...
						$SessionHostInfo = Get-AzWvdSessionHost -HostPoolName $HostpoolName -ResourceGroupName -Name $SessionHost
						if ($RoleInstance.PowerState -ne "VM running" -and $SessionHostInfo.UpdateState -eq "Succeeded") {
							# Check if the session host is allowing new connections
							Check-ForAllowNewConnections -HostPoolName $HostpoolName -ResourceGroupName $ResourceGroupName -Name $SessionHost
							# Start the Az VM
							...
							# Wait for the VM to start
							$SessionHostIsAvailable = Check-IfSessionHostIsAvailable -HostPoolName $HostpoolName -ResourceGroupName $ResourceGroupName -Name $SessionHost
			...
			if ($HostPoolUserSessions.Count -ge $AvailableSessionCapacity) {
				...
				foreach ($SessionHost in $AllSessionHosts.Name) {
					if ($HostPoolUserSessions.Count -ge $AvailableSessionCapacity) {
						...
						if ($SessionHost.ToLower().Contains($RoleInstance.Name.ToLower())) {
							# Check if the Azure VM is running and if the session host is healthy
							$SessionHostInfo = Get-AzWvdSessionHost -HostPoolName $HostpoolName -ResourceGroupName $ResourceGroupName -Name $SessionHost
							if ($RoleInstance.PowerState -ne "VM running" -and $SessionHostInfo.UpdateState -eq "Succeeded") {
				...
				if ($SessionHost.Status -ne "NoHeartbeat" -and $SessionHost.Status -ne "Unavailable") {
					if ($NumberOfRunningHost -gt $MinimumNumberOfRDSH) {
						...
						if ($SessionHost.Sessions -eq 0) {
							...
							$LogMessage = @{ hostpoolName_s = $HostpoolName; logmessage_s = "Counting the current sessions on the host $Name :$HostUserSessionCount" }
							Add-LogEntry -LogMessageObj $LogMessage -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -logType "WVDTenantScale_CL" -TimeDifferenceInHours $TimeDifference
							$ExistingSession = 0
							...
							foreach ($Session in $HostPoolUserSessions) {
								if ($Session.Name -eq $Name) {
									#Log off user
									try {
										Disconnect-AzWvdUserSession -HostPoolName $HostpoolName -ResourceGroupName $ResourceGroupName -Name $Session.Name -SessionId $Session.SessionId -NoUserPrompt -Force -ErrorAction Stop
										$ExistingSession = $ExistingSession - 1
									}
									catch {
			...
			if ($HostpoolSessionCount -ge $ScaleFactor) {
				...
				foreach ($SessionHost in $AllSessionHosts) {
					...
					if ($SessionHost.UpdateState -eq "Succeeded") {
						...
						$VMName = $Name.Split(".")[0].Split("/")[1]
						# Validating session host is allowing new connections
						Check-ForAllowNewConnections -HostPoolName $HostpoolName -ResourceGroupName $ResourceGroupName -Name $SessionHost.Name
						# Start the Az VM
						Write-Output "Starting Azure VM: $VMName and waiting for it to complete ..."
						$LogMessage = @{ hostpoolName_s = $HostpoolName; logmessage_s = "Starting Azure VM: $VMName and waiting for it to complete ..." }
						Add-LogEntry -LogMessageObj $LogMessage -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -logType "WVDTenantScale_CL" -TimeDifferenceInHours $TimeDifference
						...
						$SessionHostIsAvailable = Check-IfSessionHostIsAvailable -HostPoolName $HostpoolName -ResourceGroupName $ResourceGroupName -Name $SessionHost.Name
						if ($SessionHostIsAvailable) {
							Write-Output "$($SessionHost.Name | Out-String) session host status is 'Available'"
							$LogMessage = @{ hostpoolName_s = $HostpoolName; logmessage_s = "$($SessionHost.Name | Out-String) session host status is 'Available'" }
							Add-LogEntry -LogMessageObj $LogMessage -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -logType "WVDTenantScale_CL" -TimeDifferenceInHours $TimeDifference
						}
						else {
							...
							$LogMessage = @{ hostpoolName_s = $HostpoolName; logmessage_s = "'$($SessionHost.Name | Out-String)' session host does not configured properly with deployagent or does not started properly" }
							Add-LogEntry -LogMessageObj $LogMessage -LogAnalyticsWorkspaceId $LogAnalyticsWorkspaceId -LogAnalyticsPrimaryKey $LogAnalyticsPrimaryKey -logType "WVDTenantScale_CL" -TimeDifferenceInHours $TimeDifference
						}
