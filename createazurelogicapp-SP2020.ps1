...
.PARAMETER WebhookURI
 ...
 Example: .\createazurelogicapp.ps1  -AADTenantID "Your Azure TenantID" -SubscriptionID "Your Azure SubscriptionID" `
 -HostPoolName "Name of the HostPoolName" -PeakLoadBalancingType "Load balancing type in Peak hours" -MaintenanceTagName "Name of the Tag Name" -RecurrenceInterval "Repeat job every and select the appropriate period of time in minutes (Ex. 15)" `
...
if ($RoleAssignment.RoleDefinitionName -eq "Owner" -or $RoleAssignment.RoleDefinitionName -eq "Contributor")
{
	...
	#Creating Azure logic app to schedule job
		...
		$SessionHostsList = Get-azwvdSessionHost -ResourceGroupName $ResourcegroupName -HostPoolName $HostpoolName
		...
		elseif ($SessionHostCount -le $MinimumNumberOfRDSH) {
			...
			else { Write-Output "Configuring the scale script for the hostpool : '$HostPoolName' and will keep the minimum required session hosts in running mode." }
		}
